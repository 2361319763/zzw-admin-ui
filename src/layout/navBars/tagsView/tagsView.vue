...
import { defineAsyncComponent, reactive, onMounted, computed, ref, nextTick, onBeforeUpdate, onBeforeMount, onUnmounted, watch, getCurrentInstance } from 'vue';
...
// 2、刷新当前 tagsView：
const refreshCurrentTagsView = async (path) => {
	proxy.$tab.refreshPage(path);
	if (route.meta.link) {
		useTagsViewStore().delIframeView(route);
	}
};
// 3、关闭当前 tagsView：如果是设置了固定的（affix），不可以关闭
const closeCurrentTagsView = (v: RouteItem) => {
	const path = v || selectedTag.value;
	proxy.$tab.closePage(path).then(({ visitedViews }:any) => {
    if (isActive(path)) {
...
// 6、开启当前页面全屏
const openCurrenFullscreen = async (item: RouteItem, k: number) => {
